...
export async function GET(req: NextRequest) {
  ...
  const allowed = new Set(["pending", "processing", "sent", "failed", "cancelled"])
  const statusFilter = allowed.has(status) ? status : null

  const admin = getSupabaseAdmin()
  const q = admin
    .from("notifications")
    .select("id,channel,type,title,body,status,run_at,sent_at,attempts,last_error,meta,created_at")
    .eq("user_id", user.id)
